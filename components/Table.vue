...
const handlePre = () => {
  if (props.paging.currentPage > 1) {
    router.get(
      route().current(),
      { page: props.paging.currentPage - 1 },
      { replace: true }
    );
  }
};
...
<template>
  ...
  >
    <table
      :class="`w-full ${classes} ${sticky ? 'table-stick' : ''} ${
        mode !== 'basic' ? 'border-[1px] border-solid border-grey' : ''
      }`"
    >
      <thead v-if="headers && headers.length > 0">
        <tr :class="{ 'h-[45px]': mode === 'basic' }">
          <th
            :style="`--width: ${
              header.width ? header.width + 'px' : 'undefined'
            }`"
            :class="`${
              !$slots[`header-${fields[index]}`] && mode !== 'basic'
                ? 'header-item'
                : 'header-basic'
            } ${header?.stick ? 'column-sticky' : ''}`"
            v-for="(header, index) in headers"
            :key="index + header"
          >
            <div v-if="$slots[`header-${fields[index]}`]" class="card-header">
              <slot :name="`header-${fields[index]}`" :data="header"></slot>
            </div>
        ...
        >
          <td
            :class="`${mode !== 'basic' ? 'columns' : 'columns-basic'} ${
              propertyEmpty && !item[propertyEmpty] ? 'bg-[#fe2a2a1a]' : ''
            } ${headers[index]?.stick ? 'column-sticky' : ''}`"
            ...
            <div
              ...
              :class="{
                'columns__multi-value':
                  Array.isArray(item[field]) && mode !== 'basic',
              }"
            >
      ...
      <tfoot v-if="footers && footers.length > 0">
        <tr>
          <td
            :style="`--width: ${
              headers[index].width ? headers[index].width + 'px' : 'undefined'
            }`"
            :class="`header-item ${
              headers[index].stick ? 'column-sticky' : ''
            }`"
    ...
    <div
      v-if="Object.keys(paging).length !== 0 && paging.totalPage !== 0"
      class="pt-[50px]"
    >
      <div>
        <div
        ...
        >
          ...
          >
            <div
              ...
              class="paging-button"
              :class="{
                'paging-button__active': 1 == paging.currentPage,
              }"
            >
              <p>1</p>
            </div>
            ...
            <div
              ...
              class="paging-button"
              :class="{
                'paging-button__active': item === paging.currentPage,
              }"
