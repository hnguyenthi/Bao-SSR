    ...
    <div class="admin-box">
      <div class="admin-user__btns flex justify-start items-end gap-x-6">
        <button
          @click="handleDelete"
          type="button"
          class="btn px-4 btn-outline-primary w-fit btn-sm"
        >
          削除
        </button>
        <button
        ...
        >
          <template #active="{ data }">
            <div class="text-[14px]">
              {{
                data.active &&
                new Date(data.startdt).getTime() <= Date.now() &&
                new Date(data.enddt).getTime() >= Date.now()
                  ? "公開中"
                  : ""
              }}
            </div>
          ...
          <template #title="{ data }">
            <div @click="handleRedirect(data)" class="cursor-pointer">
              <div
                class="text-primary hover:underline text-two-line text-[14px]"
              >
                {{ data.title }}
              </div>
            </div>
          ...
          <template #regdate="{ data }">
            <div class="text-[14px]">
              {{ formatDateYYYYMMDD(data.regdate) }}
            </div>
          </template>
          <template #upddate="{ data }">
            <div class="text-[14px]">
              {{ formatDateYYYYMMDD(data.upddate) }}
            </div>
          </template>
          ...
          <template #enddt="{ data }">
            <div class="text-[14px]">{{ formatDateYYYYMMDD(data.enddt) }}</div>
          </template>
        </AdminTable>
        <!-- footer -->
      </div>
